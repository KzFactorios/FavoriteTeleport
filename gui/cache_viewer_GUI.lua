-- cache_viewer_GUI.lua
-- Advanced tree view GUI for displaying the FavoriteTeleport cache object
...
local GUI_NAME = "ft_cache_viewer_frame"
local SCROLL_NAME = "ft_cache_viewer_scroll"
local CLOSE_BTN_NAME = "ft_cache_viewer_close_btn"
...
local function add_table_tree(parent, data, depth, expand_state, path)
  ...
  for k, v in pairs(data) do
    ...
    if is_table then
      ...
      local btn = flow.add{
        type = "button",
        name = "ft_cache_viewer_toggle_" .. node_path,
...
--- Open the cache viewer GUI for a player
function CacheViewerGUI.open(player, cache, expand_state)
  ...
  add_table_tree(scroll, cache, 0, expand_state or {}, "root")
end

--- Close the cache viewer GUI for a player
function CacheViewerGUI.close(player)
  if not player or not player.valid then return end
  local gui = player.gui.screen
  if gui[GUI_NAME] then gui[GUI_NAME].destroy() end
end

return CacheViewerGUI
